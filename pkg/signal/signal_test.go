...
func testWithTermination(t *testing.T, sig os.Signal) {
	ctx, _ := signal.WithTermination(context.Background())
...
func TestWithSignals(t *testing.T) {
	ctx, _ := signal.WithSignals(context.Background(), syscall.SIGUSR1)
...
func TestWithSignalsCanceled(t *testing.T) {
	ctx, cancel := signal.WithSignals(context.Background(), syscall.SIGUSR1)
	...
	if _, err := signal.Closed(ctx); err == nil {
